...
class Pokryvala(models.Model):
    ...
    def __str__(self):
        return " %s" % self.name
    class Meta:
        verbose_name = 'Покрывала'
        verbose_name_plural = 'Покрывала'
...
def pre_save_product_slug(sender,instance, *args, **kwargs):
    if not instance.slug:
        slug = slugify(translit(instance.name, reversed=True))
        instance.slug = slug
pre_save.connect(pre_save_product_slug, sender=Pokryvala)
# -----------------------------end product-------------------------------------------------------------
#  --------------------------Gallery-------------------------------------------------------
# создание названия фотки
def image_gallary_folder(instance,filename):
    ...
    return "{0}/{1}".format(instance.slug,filename)
# фотки продукта
class PokryvalaImage(models.Model):
    product = models.ForeignKey(Pokryvala,blank=True, null=True, default=None,on_delete=models.CASCADE)
...
def pre_save_imagegallery_slug(sender,instance, *args, **kwargs):
    ...
    if not instance.slug:
        ...
        instance.slug = slug
pre_save.connect(pre_save_imagegallery_slug, sender=PokryvalaImage)
# ---------------------------------------end gallery-------------------------------------------
